...
import {
  IDBroker,
  NameGenerator, SnapSceneUtils, SnapUtils
} from '../utils/export';
...
class Item{
  ...
  static updateDraggingItem(state, x, y) {
    let {draggingSupport, scene, snapElements} = state;
    ...
    let originalX = draggingSupport.get('originalX') + diffX;
    let originalY = draggingSupport.get('originalY') + diffY;


    let activeSnapElement = null;
    let curSnap0 = null;
    if (state.snapMask && !state.snapMask.isEmpty()) {
      curSnap0 = SnapUtils.nearestSnap(snapElements, originalX, originalY, state.snapMask);
    }

    let deltaX = 0, deltaY = 0;
    if (curSnap0) {
      deltaX = curSnap0.point.x - originalX;
      deltaY = curSnap0.point.y - originalY;
      activeSnapElement = curSnap0.snap;
    }

    originalX += deltaX;
    originalY += deltaY;
    console.log(curSnap0);
